            ...
            <section className="category-container gentle-flex flex-center flex-wrap">
                    ...
                    <div className="category-wrapper gentle-flex flex-wrap">
                        {categoriesAvailable.map((everyCategory) => {
                            const { _id, categoryName, tagLine, imgUrl,filterDispatchType } = everyCategory;
                            return (
                                <Link to={`/products?categorySelected=${filterDispatchType}`} key={_id} className="link-none category-card m-4">
