...
const applyCategoryChoice = (givenState, providedProducts) => {
  ...
  return categories.length > 0
    ? [...providedProducts].filter((everyProduct) =>
        categories.includes(everyProduct.categoryName)
      )
    : [...providedProducts];
};
...
const applyGenderChoice = (givenState, providedProducts) => {
  if (givenState.gender === "Men") {
    return [...providedProducts].filter(
      (everyItem) => everyItem.wornBy === "Men"
    );
  } else if (givenState.gender === "Women") {
    return [...providedProducts].filter(
      (everyItem) => everyItem.wornBy === "Women"
    );
  } else {
    return [...providedProducts];
  }
...
const applyRatingsChoice = (givenState, providedProducts) => {
  ...
  return [...providedProducts].filter(
    (everyProduct) =>
      Number(everyProduct.ratings) >= Number(startRatings) &&
      Number(everyProduct.ratings) <= Number(endRatings)
  );
};
...
const applyPriceChoice = (givenState, providedProducts) => {
  ...
  return [...providedProducts].filter(
    (everyProduct) => Number(everyProduct.discountedPrice) < Number(priceLimit)
  );
};

const cumulativeFilters = (...participatingFilters) => {
  return (givenState, providedProducts) => {
    return participatingFilters.reduce(
      (previousFilterResult, currentFilter) =>
        currentFilter(givenState, previousFilterResult),
      providedProducts
    );
  };
};
